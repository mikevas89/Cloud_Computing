...
public class VmMonitor implements Runnable {
	...
	public void applyAdvancedPolicy(){
		...
		if(!this.getRequestQueue().isEmpty() && this.availableAdvancedVM())
			this.assignVMToUser(this.getAvailableAdvancedVM(), this.popFromRequestQueue());
	}
	...
	public boolean availableAdvancedVM(){
		...
		for (Iterator<Entry<String, VMStats>> it = this.getVmPool().entrySet().iterator();
			...
			if (vm.getVmStatus() == VMstatus.Running && vm.hasNormalCapacityAddingOneUser())
				return true;
		}
		return false;
	}
	...
	public void handlePUsers(){
		...
		while(this.pUserExists() && this.availableVMForPUser()){
			this.assignVMToUser(this.getAvailableVMForPUser(), this.getPriorityRequest());
		}
	...
	public RequestMessage getPriorityRequest(){
		...
		for (Iterator<RequestMessage> it = this.getRequestQueue().iterator();
				it.hasNext();) {
	...
	public void assignVMToUser(String vmIP, RequestMessage requestMessage){
		...
		System.out.println("Available IP found: First request is from user "+ request.getSenderID());
		...
		availableVM.setNumRegisteredUsers(availableVM
				.getNumRegisteredUsers() + 1);
		
		System.out.println("Available IP found: Sending to user "+ request.getSenderID());
	...
	public String allocateVM(Client oneClient) {
		...
		try {
			...
			// put VM to VM pool
			this.getVmPool().put(vmIP,new VMStats(newVMID, vm, vmIP, VMstatus.Booting, 0));
	...
	public void sendUserMessage(ReplyMessage message) {
		...
		new Thread() {
			public void run() {
				...
				try {
					System.out.println("sendUserMessage: Sending to "+ reply.getReceiverID());
	...
	public void logStatus(String input) {
		...
		System.out.println("Now printing VMusers");
		for (Iterator<Entry<String, ArrayList<RegisteredUser>>> it = this.getVmUsers().entrySet().iterator(); it.hasNext();) {
		...
		try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("logger.txt", true)))) {
		    out.println(input);
		}catch (IOException e) {
			e.printStackTrace();	
		}
